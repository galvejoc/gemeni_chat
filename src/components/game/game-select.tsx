    ...
    <div className="text-xl border-slate-700 border-4 rounded-lg relative">
      ...
      <form className="mx-auto md:flex block px-3 py-5 gap-4 items-center">
        ...
        <select
          ...
          className="block truncate w-full text-lg p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          {gameSelect.map((e, index) => (
            <option key={index} value={e.type}>{e.type}</option>
          ))}
        </select>
        <label className="block mb-2 mt-3 md:mt-0 text-xl font-medium text-gray-900">
          Level
        </label>
        <select
          id="level"
